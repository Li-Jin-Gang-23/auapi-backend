use aurumapi;
-- 接口信息
create table if not exists aurumapi.`interface_info`
(
...
insert into aurumapi.`interface_info` (`name`, `description`, `url`, `requestHeader`, `responseHeader`, `status`,
                                       `method`, `userId`)
...
-- 用户调用接口关系表
create table if not exists aurumapi.`user_interface_info`
(
    `id`              bigint                             not null auto_increment comment '主键' primary key,
